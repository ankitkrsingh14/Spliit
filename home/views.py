...
def home(request):
    if request.user.is_authenticated:
        ...
        member = HistoryMember.objects.filter(user=request.user).values_list(
            "group__id", "group__name").distinct()
...
def login(request):
    ...
    if request.method == 'POST':
        ...
        if len(usernumber) != 10:
            ...
            user = authenticate(
                request, username=usernumber, password=password)
            print(user)
            if user is not None:
               ...
               #  recipient_list = ["{shivathink17@gmail.com" ]
    ...
    if request.method == 'POST':
        ...
        if len(usernumber) != 10:
            ...
            user = User.objects.create_user(
                username=usernumber, email=email, password=password)
...
def group(request):
    if request.user.is_authenticated:
        group = Group.objects.filter(user=request.user)
        member = HistoryMember.objects.filter(user=request.user).values_list(
            "group__id", "group__name", "group__is_admin", "group__user__first_name", "group__user__last_name").distinct()
        context = {'group': group, 'member': member}
    ...
    if request.user.is_authenticated:
        try:
            try:
                ...
                member_total = HistoryMember.objects.filter(
                    group=group).values_list('user__id').distinct()
...
def deletegroup(request, id):
    if request.user.is_authenticated:
        user = request.user
        group = Group.objects.get(user=user, id=id)
        print(group)
        if group.is_admin == True:
            group.delete()
            return redirect('/group')
...
#     if request.user.is_authenticated:
#         user = request.user
#         admin=HistoryMember.objects.get(user=user,id=id)
#         print(admin)
#     else:
#         return redirect('/group')
    ...
    if request.user.is_authenticated:
        if request.method == 'POST':
            ...
            history_member = HistoryMember(
                user=user, group=group, amount=amount, desc=desc)
            history_member.save()
